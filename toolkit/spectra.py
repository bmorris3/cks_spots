...
class EchelleSpectrum(object):
    ...
    def __init__(self, spectrum_list, header=None, name=None, fits_path=None,
        ...
        if header is not None and time is None and 'JD' in header:
            time = Time(header['JD'], format='jd')
        elif header is not None and time is None and 'MJD' in header:
            time = Time(float(header['MJD']), format='mjd')
    ...
    @classmethod
    def from_cks(cls, path, name=None):
        """
        Load an echelle spectrum from a FITS file.
    ...
    def rv_wavelength_shift(self, spectral_order, teff=None, plot=False):
        """
        ...
        if self.model_spectrum is None:
            if teff is None:
                teff = query_for_T_eff(self.name)
            
            self.model_spectrum = get_phoenix_model_spectrum(teff)
        ...
        if plot:
            plt.figure()
            plt.plot(order.masked_wavelength + rv_shift,
                     order.masked_flux / np.median(order.masked_flux),
            ...
            plt.plot(model_slice.wavelength,
                     gaussian_filter1d(model_slice.flux, smoothing_kernel_width) /
                     np.median(gaussian_filter1d(model_slice.flux, smoothing_kernel_width)),
                     label='smooth model')
...
def sigma_clipper(fluxes):
    return sigma_clip(fluxes, sigma_lower=np.inf, sigma_upper=6,
                      stdfunc=mad_std)


def cross_corr(target_spectrum, model_spectrum, kernel_width,
               sigma_clip_fn=sigma_clipper):
    """
    ...
    sigma_clip_fn : function
        Function used to sigma clip the target spectrum before cross-correlating
    ...
    corr = np.correlate(target_spectrum.masked_flux.value /
                        np.median(target_spectrum.masked_flux.value),
                        smoothed_model_flux / np.median(smoothed_model_flux),
                        mode='same')
